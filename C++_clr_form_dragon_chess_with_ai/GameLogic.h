...
public:
	...
	moveType isMoveValid(int table, int x, int y, list<tableRelated::Move>* possibleMoves);
	void MakeMove(int tableOrigin, int xOrigin, int yOrigin, int tableTarget, int xTarget, int yTarget, moveType type, bool isNextTurn = true, Piece* (&airTable)[8][12] = GameState::airTable, Piece* (&earthTable)[8][12] = GameState::earthTable, Piece* (&undergroundTable)[8][12] = GameState::undergroundTable);
